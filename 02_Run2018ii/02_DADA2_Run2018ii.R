# 02_DADA2_Run2018ii.R
# R script for DADA2-based error correction
...
path_cutadapt1 <- "01_Cutadapt_Run2018ii/FASTQ_trim"
## Primer trimming stats
path_cutadapt2 <- "01_Cutadapt_Run2018ii/summary.tsv"
## Output directory
path_out <- "02_DADA2_Run2018ii"

# Identifiers for paired-end FASTQ files (forward and reverse)
fastq_str_fwds <- "R1_001_trim.fastq.gz"
fastq_str_revs <- "R2_001_trim.fastq.gz"

# Identifiers of FASTQ files before and after quality filtering and trimming
fastq_str_pre <- "trim"
fastq_str_post <- "qlt"

# Plot read quality profiles before quality filtering and trimming
## Output directory
path_qprfl <- paste0(path_out, "/01_qlt_prfl")
dir.create(path_qprfl, recursive = TRUE)
## Lists of input FASTQ file names, excluding samples with zero reads
fwds <- path_cutadapt1 %>% 
...
revs <- path_cutadapt1 %>% 
  ...
  sort()
## Retrieve sample names — may vary depending on directory structure and 
## file naming
sample_names <- fwds %>% 
...
## Lists of output FASTQ file names (forward and reverse)
...
## Set `minLen` to 90 bp, considering MiFish amplicons average ~172 bp and
## require a ≥12 bp overlap for successful merging
...
## Identifiers of zero-read samples, if any, after quality filtering
zero_read_smpl2 <- which(qfltrd[, "reads.out"] == 0) %>% 
  names()
## Lists of input FASTQ file names, excluding zero-read samples (forward and 
## reverse)
...
## Length filtering: discard sequences that fall outside the expected MiFish 
## amplicon size range (160–190 bp)
...
# Create a summary table for the Cutadapt–DADA2 pipeline
