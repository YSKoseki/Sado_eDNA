# 05_Mergelib.R
# An R script to merge data of different libraries (years)
# R 4.2.2
...
# Input data
path_in <- list(
  lib2018i = "./01_Lib2018i/03_gmmDenoise_Lib2018i/ASV_tab_gmm.rds",
  lib2018ii = "./02_Lib2018ii/03_gmmDenoise_Lib2018ii/ASV_tab_gmm.rds",
  lib2019 = "./03_Lib2019/03_gmmDenoise_Lib2019/ASV_tab_gmm.rds",
  lib2022 = "./04_Lib2022/03_gmmDenoise_Lib2022/ASV_tab_gmm.rds"
)
## Output directory
path_out <- "05_Mergelib"
...
ASV_tab_nest <- ASV_tab_lis[c("lib2018i", "lib2018ii")] %>% 
  dada2::mergeSequenceTables(tables = .) %>%
  list(
    lib2018 = .,
    lib2019 = ASV_tab_lis[["lib2019"]],
    lib2022 = ASV_tab_lis[["lib2022"]]
  ) %>% 
  tibble(
    yr = names(.) %>% str_replace("lib", "") %>% as.integer(),
    asvtbl = .
  ) %>% 
  mutate(
